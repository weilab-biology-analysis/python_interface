...
class TextCNN(nn.Module):
    def __init__(self, config):
        ...
        self.filter_sizes = [int(fsz) for fsz in config.filter_sizes.split(',')]
        self.embedding = nn.Embedding(vocab_size, embedding_dim)

        self.convs = nn.ModuleList(
            [nn.Conv2d(1, filter_num, (fsz, embedding_dim)) for fsz in self.filter_sizes])
    ...
    def forward(self, x):
        ...
        return output, attention, acid_count
